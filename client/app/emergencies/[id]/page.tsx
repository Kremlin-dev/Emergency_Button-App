...
const EmergencyId = () => {
  ...
  useEffect(() => {
    ...
    const selectedIncident = emergencyData.find((item) => item.id === incidentId);
    ...
    if (!selectedIncident) {
      ...
      const storedActions = localStorage.getItem(`actions-${incidentId}`);
      const storedStatus = localStorage.getItem(`status-${incidentId}`);
  ...
  const addAction = () => {
    if (newAction.trim() !== "") {
      ...
      setNewAction("");
      localStorage.setItem(`actions-${incident?.id}`, JSON.stringify(updatedActions)); // Save to localStorage
    }
  };
  ...
  return (
    <main className="flex flex-col md:flex-row justify-center p-4 gap-6">
      ...
      <div className="w-full md:w-1/3 bg-gray-100 p-4 rounded-lg shadow-md">
        ...
        {/* Actions Input */}
        <label className="block text-sm font-semibold text-gray-700">Actions Taken:</label>
        ...
        <ul className="mt-3 list-disc pl-5 text-sm text-gray-700">
          {actions.map((action, index) => (
            <li key={index} className="mt-1">{action}</li>
          ))}
        </ul>

        {/* Status Select */}
        <label className="block text-sm font-semibold text-gray-700 mt-4">Status:</label>
        <select
          value={status}
